...
@NamedQueries({
	...
	@NamedQuery(
		...
		query = "SELECT f FROM FondsCreator f WHERE f.systemId = ?1"
	),
	@NamedQuery(
		name = Queries.fondsCreatorFindByFondsCreatorId,
		query = "SELECT f FROM FondsCreator f WHERE f.fondsCreatorId = ?1"
	),
	@NamedQuery(
		name = Queries.fondsCreatorFindAllByFondsCreatorName,
		query = "SELECT f FROM FondsCreator f where f.fondsCreatorName LIKE ?1"
	),
	@NamedQuery(
		name = Queries.fondsCreatorFindAllByDescription,
		query = "SELECT f FROM FondsCreator f where f.description LIKE ?1"
	)
})
